...
def paint(thickness, colors, direction):
    ...
    for i in range(0, 400, thickness):
        # colors the rows
        if direction:
            Line(0, i, 400, i, lineWidth=thickness,
                 opacity=50, fill=choice(colors))
        # colors the columns
        else:
            Line(i, 0, i, 400, lineWidth=thickness,
                 opacity=50, fill=choice(colors))
...
def get_direction():
    ...
    rows_or_cols = app.getTextInput(
        "Would you like to paint rows or columns first?")
    ...
    while True:
        if rows_or_cols == "rows":
            ...
            rows_or_cols = app.getTextInput(
                "Please either type 'rows' or 'columns'")


def get_colors(direction):
    # Prompts the user to enter the colors they want
    colors = app.getTextInput(
        f"What colors would you like your {'rows' if direction else 'columns'} to be? (eg blue,green,orange)")
...
def get_thickness(direction):
    ...
    thickness = app.getTextInput(
        f"How {'high' if direction else 'wide'} would you like your {'rows' if direction else 'columns'} to be?")
